...
func (dto *SearchPostsDTO) Validate() error {
	...
	case "author":
		if dto.SearchValue == "" {
			return fmt.Errorf("%w: author search value cannot be empty", custom_err.ErrInvalidInput)
		}

	case "category_id":
		if dto.SearchValue == "" {
			return fmt.Errorf("%w: category_id search value cannot be empty", custom_err.ErrInvalidInput)
		}
